...
function renderTasks(){const m=machineById(curMid);
    ...
    return `<div class="tcard ${st}" data-i="${i}">
      ...
      <div class="tc-grid">
        ...
        <div class="tc-fld grow">
          ...
          <div class="prgrow">
            <button type="button" class="stpbtn pstp" data-pstp="-1" aria-label="ลด 1%">−</button>
            <input class="prg edit" type="range" min="0" max="100" step="1" value="${p}">
            <button type="button" class="stpbtn pstp" data-pstp="1" aria-label="เพิ่ม 1%">＋</button>
          </div>
        </div>
      </div>
  ...
  $("taskBody").querySelectorAll(".tcard:not(.locked)").forEach(card=>{const i=+card.dataset.i;const T=()=>machineById(curMid).tasks[i];
    ...
    const setProg=(v,ren)=>{v=Math.max(0,Math.min(100,Math.round(+v||0)));const t=T();t.prog=v;if(+rg.value!==v)rg.value=v;
